...
# 读取sql
for SQL in ['SFW']:
    for file in ['Wiki_Text', 'legal_case', 'disease', 'drug', 'institutes', 'player', 'finance'][:1]:
        dic = {"p":[],"r":[],"f1":[]}
        if SQL == 'SF':
            with open('../benchmark/Query/'+file+'/SELECT_FROM.sql', 'r', encoding='utf-8') as f:
                content = f.read()
            folder = './results/'+file+'/SF/'
        else:
            with open('../benchmark/Query/'+file+'/SELECT_FROM_WHERE.sql', 'r', encoding='utf-8') as f:
                content = f.read()
